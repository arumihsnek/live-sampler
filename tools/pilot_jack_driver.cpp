...
struct Driver final {
    ...
    std::string mode; std::string wav_path; std::string sampler_name = "live-sampler"; double sr=48000.0; uint32_t quantum=1024; std::atomic<bool> done{false};
    ...
    std::atomic<uint64_t> play_on_event_frame{UINT64_MAX}; std::atomic<uint64_t> rec_on_event_frame{UINT64_MAX};
};
...
int main(int argc,char**argv){ Driver d; d.mode=argc>1?argv[1]:"rec120"; d.wav_path=argc>2?argv[2]:"output.wav"; const char* configured_name=std::getenv("PILOT_SAMPLER_NAME"); if(configured_name != nullptr) d.sampler_name=configured_name; const bool seq66_mode=d.mode=="seq66"; const bool return_graph=!(argc>3 && std::string(argv[3])=="--no-return"); if(d.mode=="elastic60")d.play_off=d.play_on+240000; else if(d.mode=="elastic120")d.play_off=d.play_on+144000; else if(d.mode=="elastic240")d.play_off=d.play_on+96000; else if(d.mode=="dynamic"){d.play_off=484096;d.dynamic_a=d.play_on+24000;d.dynamic_b=d.play_on+80000;} else if(d.mode=="slotreplace"){d.replace_on=d.event_base+144000;d.replace_off=d.event_base+240000;d.play_on=d.event_base+336000;d.play_off=d.event_base+432000;}
    ...
    for(int i=0;i<50;++i){ int rc1=jack_connect(d.client,jack_port_name(d.ao1),(d.sampler_name+":audio_in_1").c_str()); int rc2=jack_connect(d.client,jack_port_name(d.ao2),(d.sampler_name+":audio_in_2").c_str()); int rc3=return_graph ? jack_connect(d.client,(d.sampler_name+":audio_out_1").c_str(),jack_port_name(d.ai1)) : 0; int rc4=return_graph ? jack_connect(d.client,(d.sampler_name+":audio_out_2").c_str(),jack_port_name(d.ai2)) : 0; int rc5=jack_connect(d.client,jack_port_name(d.mo),(d.sampler_name+":midi_in").c_str()); if(i==0) std::cerr<<"connect_rc="<<rc1<<","<<rc2<<","<<rc3<<","<<rc4<<","<<rc5<<" return_graph="<<(return_graph?1:0)<<"\n"; if((rc1==0 || rc1==17) && (rc2==0 || rc2==17) && (rc5==0 || rc5==17)) break; std::this_thread::sleep_for(std::chrono::milliseconds(100)); }
    if(!seq66_mode){ jack_transport_locate(d.client, 0); jack_transport_start(d.client); } auto start=std::chrono::steady_clock::now(); auto last_report=start; while(!d.done.load()){ std::this_thread::sleep_for(std::chrono::milliseconds(50)); auto now=std::chrono::steady_clock::now(); if(now-last_report>std::chrono::seconds(1)){std::cerr<<"driver_frame="<<d.last_frame<<"\n";last_report=now;} if(now-start>std::chrono::seconds(30)){std::cerr<<"driver timeout\n";break;} } if(!seq66_mode) jack_transport_stop(d.client); jack_deactivate(d.client); const bool bbt_ok=validate_bbt(d); wav(d.wav_path,d.rec_l,d.rec_r,d.rec_frames,static_cast<uint32_t>(d.sr)); std::cout<<"PLAY_ON_EVENT_FRAME="<<d.play_on_event_frame.load(std::memory_order_relaxed)<<" REC_ON_EVENT_FRAME="<<d.rec_on_event_frame.load(std::memory_order_relaxed)<<"\n"; std::cout<<"DRIVER mode="<<d.mode<<" sample_rate="<<d.sr<<" quantum="<<d.quantum<<" captured_frames="<<d.rec_frames<<" output="<<d.wav_path<<"\n"; jack_client_close(d.client); return bbt_ok?0:4; }
