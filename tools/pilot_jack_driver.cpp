...
static void wav(const std::string& path,const std::vector<float>& l,const std::vector<float>& r,std::size_t n,uint32_t sr) {
    std::ofstream f(path,std::ios::binary); const uint32_t data=static_cast<uint32_t>(n*4*2), riff=36+data, fmt=16, audio=3, channels=2, rate=sr, bytes=rate*channels*4, align=channels*4, bits=32;
