...
SamplerEngine::~SamplerEngine() {
    ...
    for (auto& slot : slots_) {
        delete[] (slot.current ? slot.current->left : nullptr);
        delete[] (slot.current ? slot.current->right : nullptr);
        delete slot.current;
        slot.current = nullptr;
    }
    SampleBuffer* p = nullptr;
    while (publish_queue_.pop(p)) { delete[] p->left; delete[] p->right; delete p; }
    while (retire_queue_.pop(p)) { delete[] p->left; delete[] p->right; delete p; }
}
...
void SamplerEngine::worker_loop() {
    ...
    if (pending != nullptr) {
        while (!publish_queue_.push(pending)) std::this_thread::sleep_for(std::chrono::milliseconds(1));
    }
    drain_retire_queue();
}
...
void SamplerEngine::drain_publish_queue() noexcept {
    ...
    while (publish_queue_.pop(sample)) {
        ...
        if (note < 0 || note >= 128) { retire_if_unused(sample); continue; }
        auto* old = slots_[note].current;
        slots_[note].current = sample;
        capture_pending_ = false;
        if (old != nullptr) retire_if_unused(old);
    }
}
...
void SamplerEngine::retire_if_unused(SampleBuffer* sample) noexcept {
    if (sample == nullptr || sample->retire_queued || sample_referenced(sample)) return;
    if (retire_queue_.push(sample)) sample->retire_queued = true;
}
